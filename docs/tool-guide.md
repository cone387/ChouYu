...
- 单次回答最多进行六轮模型请求（含工具续答），覆盖查清单、查任务、读详情、确认修改和结果说明，仍限制循环上限。

## AI 任务助手

已接入 `list_task_projects`、`search_tasks`、`get_task`、`update_task` 和 `complete_task`，详见 [使用与验证](task-assistant.md)。任务写操作始终逐次确认；普通查询遵循设置中的工具权限模式。工具可以提供 `prepare`，在授权前构造只读预览并捕获版本，授权后执行同一份操作。
