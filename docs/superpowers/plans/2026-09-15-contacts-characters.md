...
2. **占位符**：Task 3/7/9 中标注「实现期修正」的三处代码笔误已显式给出修正说明，无 TBD。
   ...
   **Task 7 实现期修正（已镜像）**：(a) 落地适配（34bfd2d）：`SessionWorkspace` 类型导入路径是 `../../shared/types`；`.character-context-chip` 样式须用设计 token（DesignTokens.regression.test.ts 禁裸 px）；`openCharacterChat` 从 **FULL sessions** 过滤（计划的 visibleSessionsRef 版本在新角色场景总是新建会话，无法回到最近会话）；InputArea 的 `fetchModels` prop 经 `fetchModelsRef` 间接读取（否则每渲染都触发重取）。(b) 质量审修正（06cc05c）：模型选择器与 `/model` 在自定义角色活跃时曾直写全局配置——改为 character-aware `handleModelChange`（自定义角色 → `characters.update` 携带完整 name/avatar/soulMd/providerProfileId + 新 model；内置 → `db.saveConfig` + `setConfig`）；InputArea 取模型列表 effect deps 改 `[refreshModels, sessionId]`（会话切换重取）；新增 `handleCharactersDeleted` 对被删角色的每个会话先 `stopSessionResponse`（try/catch）再 `applyWorkspace(workspace, true)`（中止在途生成）；ContactsView 的 `'chouyu'` 字面量改 `DEFAULT_CHARACTER_ID`、确认删除按钮 `disabled={busy}`；补 4 条源断言测试。
   **Task 8 实现期修正（已镜像）**：(a) `ProviderProfilesCard` 挂载为 AI 面板内既有卡片的**兄弟节点**（组件自带 `<section class="settings-card">`，嵌套会双边框）；新增「新建档案」在自定义档案数 ≥ `MAX_PROVIDER_PROFILES` 时禁用并提示上限（镜像 8fc81cf 主进程守卫，否则用户只能靠报错发现上限）。(b) 计划未列 CSS 任务但 `.provider-profile-form`/`.settings-hint`/`.danger` 无既有样式——在 Settings.css 末尾追加最小样式块；**间距必须用 `var(--space-*)` token**（DesignTokens.regression.test.ts 同样覆盖 Settings.css 的 margin/gap/padding 裸 px，计划原以为只限 ChatPanel.css）。
   **Task 9 实现期修正（已镜像）**：(a) 计划的表单消失等待 `document.querySelector('[data-contacts-form]') === false` 恒不可能为真（querySelector 返回 null/Element），改为 `!document.querySelector(...)`。(b) **插入位置从「runChatRuntimeSmoke 之后」改为之前（storage 与 chat 之间）**：chat-smoke 的拖拽断言在本机历史恒失败并中止整链（app.exit(1)），放在其后则 contacts 冒烟本机永远跑不到；前置后仍须自足准备面板状态。(c) 自足准备：`webContents.send('open-chat-panel')` + 等待面板可见；点击 `[aria-label="窗口模式"]` → `[data-workspace-mode-option="workspace"]` 等 `data-window-mode === 'workspace'`（chat 页仅 workspace 模式渲染导航）。(d) 结束时回聊天页（等 `.input-textarea`）后**关闭面板等卸载**——chat-smoke 依赖首次 open 时全新挂载 ChatPanel，本冒烟若留下已挂载面板会破坏其早期等待。(e) 冒烟结束后新增竞态守卫（b2372e7）：点击 `[data-contacts-item]`/`[data-contacts-delete]` 前先 waitForRenderer 该控件——ContactsView 的 `characters:changed` 列表刷新是异步的，表单关闭等待通过时列表可能尚未渲染新角色（规格复审实测间歇失败）。(f) 超范围必要修复：`src/main/smoke/chat-smoke.ts` 导航计数断言 4→5（通讯录是第 5 个导航按钮，Task 7 漏更，不修则失败点先于拖拽基线、掩盖基线判断）；`scripts/smoke-electron.js` 迁移版本断言 3→4（STORE_VERSION 已升 4，Task 3 漏更 runner）。
   **终审修正（已镜像，dabb192）**：整体终审发现 `handleSend` 的发送门禁 `!isAIConfigured(config)` 只看全局配置——仅配好自定义角色（全局 Base URL/API Key/模型为空）时无法发送且提示误导，破坏「角色凭据独立于全局配置」的核心承诺。修正：派生 `customCharacterActive`，门禁改为 `!customCharacterActive && !isAIConfigured(config)`（内置角色行为不变）；onboarding 卡片在自定义角色活跃时不渲染（不再盖住角色 chip）；补 3 条源断言测试。
