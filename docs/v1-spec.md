...
以下内容是最初制定 V1 规格时的排除项。截图、剪贴板、主动提醒和插件系统已在后续版本中加入；当前准确状态请以 [当前功能状态](current-status.md) 为准。
...
- [x] 启动后桌面出现角色，有 idle 浮动动画
- [x] 可自由拖拽，到边缘自动吸附
- [x] Alt+Space 弹出输入框，Esc 收起
- [x] 输入文字后 AI 流式回复，显示在气泡中
- [x] 角色根据状态切换动画（idle/thinking/talking/sleeping）
- [x] 对话有记忆（关闭重开后能接着聊）
- [x] 通过 SOUL.md 控制角色性格和说话风格
- [x] 托盘右键可退出/设置
- [x] 设置中可配置 API Key 和模型
- [x] 可打包为 Windows 安装包

以上为功能实现验收；安装/卸载、自动更新、多显示器和真实 Provider 等发布验收项见 [0.6.0 发布检查清单](release-checklist-v0.6.0.md)。
