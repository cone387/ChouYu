...
export function JournalTasks({ date, query, revision, onOpen, onOrganize }: { date: string; query: string; revision: number; onOpen(id: string): void; onOrganize(): void }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      void window.electronAPI.journal.tasks({ ...journalRange(date), query, offset }).then(value => {
        if (!active) return
        if (offset && offset >= value.total) { setOffset(Math.max(0, Math.floor((value.total - 1) / 50) * 50)); return }
  ...
  return <section className="journal-tasks" aria-label="事项视图">
    ...
    <ol key={`${date}:${query}:${offset}`} className="journal-task-list" tabIndex={0} aria-label="事项列表">{page?.items.map(task => <li key={task.id}>
    ...
    {page && page.total > 50 && <nav className="journal-pagination" aria-label="事项分页"><button disabled={!offset} onClick={() => { setOffset(Math.max(0, offset - 50)); setPage(null) }}>上一页</button><span>第 {offset / 50 + 1} / {Math.ceil(page.total / 50)} 页</span><button disabled={offset + 50 >= page.total} onClick={() => { setOffset(offset + 50); setPage(null) }}>下一页</button></nav>}
  </section>
}
