...
export default function TaskCollectionDialog({ title, name, onNameChange, groups, groupId, onGroupChange, busy, error, editing, nameLabel, formClassName = '', onSubmit, onClose }: Props) {
  ...
  return <div className="tasks-dialog-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget && !busy) onClose() }}>
    <div ref={dialogRef} className="tasks-dialog tasks-collection-dialog" role="dialog" aria-modal="true" aria-labelledby="tasks-collection-title" onKeyDown={event => {
      ...
      <header className="tasks-dialog-header"><h2 id="tasks-collection-title">{title}</h2><button type="button" disabled={busy} aria-label={`关闭${title}弹窗`} onClick={onClose}>×</button></header>
      <form className={`tasks-form tasks-collection-form ${formClassName}`} onSubmit={onSubmit}>
        {error && <p role="alert" className="tasks-error">{error}</p>}
        <label>{nameLabel ?? (groups ? '清单名称' : '分组名称')}<input aria-label={nameLabel} value={name} maxLength={groups ? undefined : 50} required onChange={event => onNameChange(event.target.value)} placeholder={groups ? '输入清单名称' : '输入分组名称'} /></label>
