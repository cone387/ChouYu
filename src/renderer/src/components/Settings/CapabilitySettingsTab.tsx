  ...
  return (
    <div className="settings-pane capability-settings-pane">
      <div className="settings-pane-heading">
        <h2>能力中心</h2>
        <p>主记忆引擎只能选择一个；Embedding 是独立的可选能力。</p>
      </div>
        ...
        return <section className="capability-group" key={group.kind}>
          ...
          <div className="capability-list">
            ...
            {items.map((capability) => {
              ...
              <div className="capability-card-side"><div className="capability-badges"><span className={capability.networkAccess ? 'network' : 'local'}>{capability.networkAccess ? '需要网络' : '完全本地'}</span><span>{capability.sendsMemoryData ? '会发送记忆数据' : '不发送数据'}</span></div><button type="button" className={isPrimaryActive ? 'selected' : ''} onClick={() => { selectCapability(capability) }} disabled={isPrimaryActive}>{isPrimaryActive ? '当前使用' : '选择此能力'}</button></div>
            </article>
            })}
          </div>
        </section>
