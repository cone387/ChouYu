...
const GROUPS: Array<{ kind: CapabilityInfo['kind']; title: string; description: string }> = [
  ...
  { kind: 'memory-sync', title: '记忆同步', description: '连接远程记忆服务；所有上传都需要显式确认。' }
]
  ...
  return (
    <div className="settings-pane capability-settings-pane">
      <div className="settings-pane-heading">
        <h2>能力中心</h2>
        <p>独立选择记忆引擎、Embedding 和记忆同步。每项能力都标明网络和数据边界。</p>
      </div>
        ...
        return <section className="capability-group" key={group.kind}>
          ...
          <div className="capability-list">
            ...
            {group.kind === 'memory-sync' && <article className={`capability-card${config.memorySyncProvider === 'none' ? ' active' : ''}`}>
              <div className="capability-card-main"><span className="capability-state">{config.memorySyncProvider === 'none' ? '已启用' : '可选'}</span><strong>不启用同步</strong><p>只使用本地记忆，不连接远程服务。</p></div><div className="capability-badges"><span className="local">完全本地</span><span>不发送数据</span></div>
            </article>}
            {items.map((capability) => <article key={capability.id} className={`capability-card${capability.active ? ' active' : ''}`}>
              <div className="capability-card-main"><span className={`capability-state${capability.active ? ' on' : ''}`}>{capabilityState(capability)}</span><strong>{capability.name}</strong><p>{capability.description}</p></div>
              <div className="capability-card-side"><div className="capability-badges"><span className={capability.networkAccess ? 'network' : 'local'}>{capability.networkAccess ? '需要网络' : '完全本地'}</span><span>{capability.sendsMemoryData ? '会发送记忆数据' : '不发送数据'}</span></div><button type="button" className={capability.active ? 'selected' : ''} onClick={() => { selectCapability(capability) }} disabled={capability.active}>{capability.active ? '当前使用' : '选择此能力'}</button></div>
            </article>)}
          </div>
        </section>
      })}
