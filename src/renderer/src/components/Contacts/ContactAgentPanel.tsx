...
import { AGENT_STATUS, DEFAULT_AGENT_SETTINGS, type AgentOverview, type AgentRunDetail, type AgentSettings } from '../../../../shared/agents'
...
export function ContactAgentPanel({ characterId, name, selectedTab, onTabChange, compact = false }: {
  ...
  const refresh = async () => {
    ...
    } catch (error) { if (mounted.current && request === epoch.current) setError(String(error instanceof Error ? error.message : error)) }
  }
  useEffect(() => {
  ...
  return <section className={`contact-agent${compact ? ' contact-agent-compact' : ''}`} aria-label={`${name}的持续工作`} data-contact-agent={characterId}>
    ...
    {data && tab === 'work' && <>
      ...
      {latest && <div className="agent-latest"><span className="agent-caption">最近的成果 · {time(latest.createdAt)}</span><h4>{latest.title}</h4><p>{latest.nextStep || latest.body.slice(0, 180)}</p><button type="button" onClick={() => { setTab('history'); void perform(async () => setDetail(await window.electronAPI.agents.detail(characterId, latest.runId))) }}>追溯这次工作 →</button></div>}
      {!latest && <p className="agent-empty">先给我一个长期方向和资料来源。每轮的发现、依据与下一步都会留在这里。</p>}
      <form onSubmit={e => { e.preventDefault(); void perform(async () => { const result = await window.electronAPI.agents.save(characterId, settings); setDraft(result.settings); setSources(result.settings.sources.join('\n')) }) }}>
        ...
        <div className="agent-actions"><button data-agent-save type="submit" className="primary" disabled={busy}>保存工作设置</button><button data-agent-run type="button" disabled={busy || Boolean(dirty) || !data.settings.goal || Boolean(active)} onClick={() => void perform(() => window.electronAPI.agents.run(characterId))}>现在工作一轮</button>{(data.settings.enabled || active) && <button type="button" disabled={busy} onClick={() => void perform(async () => { await window.electronAPI.agents.pause(characterId); setDraft(previous => ({ ...previous, enabled: false })) })}>暂停工作</button>}</div>
        {dirty && <p className="agent-caption">设置尚未保存。保存会停止当前未完成的工作。</p>}
      </form>
      <p className="agent-caption">今日已调用 {data.callsToday}/{data.settings.dailyCalls} 次{data.settings.enabled && !active ? ` · 下轮 ${data.callsToday >= data.settings.dailyCalls ? '明日额度恢复后' : data.nextAt > Date.now() ? time(data.nextAt) : '等待调度'}` : ''}。连续失败 3 轮会自动暂停。</p>
    </>}
    {data && tab === 'history' && <>
      ...
      <ol className="agent-history">{data.runs.map(run => <li key={run.id}><button type="button" disabled={busy} onClick={() => void perform(async () => setDetail(await window.electronAPI.agents.detail(characterId, run.id)))}><span>{run.summary || AGENT_STATUS[run.status]}</span><small>{time(run.createdAt)} · {AGENT_STATUS[run.status]}</small></button></li>)}</ol>
