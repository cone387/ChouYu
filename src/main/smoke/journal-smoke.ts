    ...
    if (directory) {
      ...
      for (const theme of ['light', 'dark']) {
        for (const width of [1024, 560]) {
          ...
          await journal.webContents.executeJavaScript("document.querySelector('[aria-label=记录设置]').focus(); document.querySelector('[aria-label=记录设置]').click()")
          ...
          await waitForRenderer(journal, "!document.querySelector('.journal-settings-dialog') && !document.querySelector('[aria-label=记录设置]').matches(':focus')")
          await journal.webContents.executeJavaScript("[...document.querySelectorAll('.journal-view-nav button')].find((button) => button.textContent === 'AI 用量').click()")
