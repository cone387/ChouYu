...
describe('TasksStore', () => {
  ...
  test('归档退出日常视图并暂停提醒，显式查看和恢复保留原任务', () => {
    ...
    expect(store.claimDueReminders(2).map(item => item.task.id)).toEqual([task.id])
  ...
  test('claimDueReminders 只领取一次且排除已完成/无提醒', () => {
    ...
    expect(first.map(item => item.task.id)).toEqual([due.id])
    expect(first[0].task.remindFiredAt).toBe(now)
    expect(store.claimDueReminders(now + 6_000).map(item => item.task.id)).toEqual([later.id])
  ...
  test('修改提醒时间会重置已触发标记', () => {
    ...
    expect(store.claimDueReminders(2_000).map(item => item.task.id)).toEqual([task.id])
    const updated = store.updateTask(task.id, { remindAt: 10_000 })
    expect(updated.remindFiredAt).toBeNull()
    expect(store.claimDueReminders(11_000).map(item => item.task.id)).toEqual([task.id])
    store.close()
  })
