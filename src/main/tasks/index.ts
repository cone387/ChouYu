...
export function initializeTasks(options: TasksModuleOptions): void {
  ...
  ipcMain.handle('tasks:restoreBackup', async (event, token: string, settings: TaskUISettings) => {
    ...
    const restored = store!.restoreBackup(pending.backup)
    pendingBackups.delete(event.sender.id)
  ...
  ipcMain.handle('tasks:deleteGroup', (_event, id: string, deleteContents?: boolean) => store!.deleteGroup(id, deleteContents))
  ...
  ipcMain.handle('tasks:deleteProject', (_event, id: string) => store!.deleteProject(id))
